  ...
  useEffect(() => {
    const fetchSeries = async () => {
      try {
        ...
        const mappedSeries: MovieOrSeries[] = response.data
          .filter((item: any) => item.media_type === 'tv') // Filter only TV shows
          .map((item: any) => ({
            id: String(item.id),
            title: item.name || 'N/A', 
            thumbnail: getTmdbImageUrl(item.poster_path, 'w500', 300, 450), // Use getTmdbImageUrl
            genre: mapGenreIdsToNames(item.genre_ids, 'tv'), // Map genre IDs
            year: new Date(item.first_air_date || '0').getFullYear() || 0,
            rating: item.vote_average ? parseFloat(item.vote_average.toFixed(1)) : 0,
            media_type: 'tv',
          }));
        setSeries(mappedSeries.filter(item => item.year > 0));
      } catch (err: any) {
        console.error("Failed to fetch series:", err);
        setError("خطا در بارگذاری سریال‌ها. لطفاً دوباره تلاش کنید.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white text-center p-8">
        <h2 className="text-3xl font-extrabold text-[#09f]">در حال بارگذاری سریال‌ها...</h2>
      </div>
    );
  }
  ...
  return (
    <div className="w-full p-8 bg-gray-900 bg-opacity-80 rounded-2xl shadow-2xl border border-gray-700 backdrop-filter backdrop-blur-sm text-white overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {series.length > 0 ? series.map(renderSeriesCard) : <p className="col-span-full text-center text-gray-400">سریالی یافت نشد.</p>}
      </div>
    </div>
  );
};
